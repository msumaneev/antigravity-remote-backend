...
async function checkDeviceProjectAccess(msg, device) {
    if (!device)
        return true; // Before auth is fully set up, let AUTH messages pass
    if (!device.allowed_project_id)
        return true; // Unrestricted admin device
    const allowedProjectId = device.allowed_project_id;
    ...
    if (convId) {
        ...
        try {
            ...
            if (!project)
                return false;
            const findInConversations = (convs) => {
                for (const c of convs) {
                    if (c.id === convId)
                        return true;
                    if (c.subagents && findInConversations(c.subagents))
                        return true;
                }
                return false;
            };
...
function setupRoutes(app, wss) {
    ...
    app.get('/', async (req, res) => {
        try {
            ...
            const html = (0, chatHtml_1.getChatHtml)(token);
    ...
    app.post('/api/devices/:id/restrict', (req, res) => {
        try {
            const requester = req.device;
            if (!requester || requester.deviceId !== 'local-admin') {
                res.status(403).json({ error: 'Forbidden: Only local administrator can access this endpoint' });
            ...
            const success = (0, auth_1.restrictDevice)(req.params.id, projectId || null);
            res.json({ success });
        }
        catch (err) {
            res.status(500).json({ error: err.message });
    ...
    wss.on('connection', (ws) => {
        ...
        ws.on('message', async (message) => {
            try {
                ...
                const isAllowed = await checkDeviceProjectAccess(msg, ws.device);
                if (!isAllowed) {
